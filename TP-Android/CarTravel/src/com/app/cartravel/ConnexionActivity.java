...
public class ConnexionActivity extends Activity {
	...
	protected void onResume() {
		...
		if(dernierConnecte != null){
			// Si la dernière personne à s'être connecté est encore connectée, on ouvre l'application
			if(dernierConnecte.getEstConnecte()){
				...
				// Si la dernière personne à s'être connecté n'est plus connectée, 
				// on met son courriel dans la barre de courriel
				((EditText)this.findViewById(R.id.login_username)).setText(dernierConnecte.getCourriel());
			}
		}
	}
	...
	public void btnConnexion(View source){
		// Vérifier si la combinaison usager/mot de passe existe sur le service web
	...
	private class ConnexionTask extends AsyncTask<String, Void, Utilisateurs>{
		...
		protected Utilisateurs doInBackground(String... params) {
			...
			try{
				...
				URI uri = new URI("http",Util.WEB_SERVICE, Util.REST_UTILISATEUR + "/" + m_Utilisateur.getCourriel() + 
						Util.REST_CONNEXION, "password=" + m_Utilisateur.getMotDePasse(), null);
		...
		protected void onPostExecute(Utilisateurs result) {
			if(m_Exp == null && result != null){
				...
				dataSource.open();
				Utilisateurs utilisateurExistant = dataSource.getUtilisateur(result.getCourriel()); 
				if( utilisateurExistant == null){
					// Récupérer l'usager sur le service web et l'ajouter en local
				...
				if (checkPlayServices()) {
		            ...
		            Log.i("Connexion", "No valid Google Play Services APK found.");
		        }
				...
				Toast.makeText(m_Context, m_Context.getResources().getText(R.string.toast_erreur_connexion), Toast.LENGTH_SHORT).show();
