...
public class ParcoursPassager implements java.io.Serializable {
	 ...
	 * pour la gestion plus tard en vérifiant si tous les passagers sont des utilisateurs
	 * de l'app. ou non.
	 */
	...
	public void setListIdPassager(String IdPassager, int Participation, int NbrPassagers) {
		//Si un utilisateur se propose en tant que passager, le
		//paramètre Participation sera égale à 1, s'il décide de s'enlever alors
		//le paramètre Participation sera égale à 0;
		if (Participation == 1) {
			int i = 1;
			
			//On s'assure que le nouveau passager s'ajoute à la fin de la liste et non au début.
			...
			int i2 = i + 1;
			//On vérifie si lors de la participation à un covoiturage ou lors d'une création le nombre
			//de passagers est supérier à 1. Si oui on supprime les éléments suivant l'utilisateur qui
			//désire ne plus particper au covoiturage. Donc, ceux qui sont égales à zéro.
			if (m_List_Id_Passager.get(i2).equals("0")) {
				...
				//Nombre de passagers diminue du nombre de passagers passé en paramètre dans ce cas-ci.
				m_Nbr_Passagers -= NbrPassagers;
