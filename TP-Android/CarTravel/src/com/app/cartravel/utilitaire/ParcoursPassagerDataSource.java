...
public class ParcoursPassagerDataSource {
	...
	public void update(ParcoursPassager p_ParcourPassager) {
		...
		m_Db.update(DatabaseHelper.TABLE_PARCOURS_PASSAGER, row,
				DatabaseHelper.COL_ID_PARCOURS_PASSAGER + "='" + p_ParcourPassager.getIdParcoursPassager() + "'", null);
	}
	
	public void delete(String p_Id) {
		m_Db.delete(DatabaseHelper.TABLE_PARCOURS_PASSAGER,
				DatabaseHelper.COL_ID_PARCOURS_PASSAGER + "='" + p_Id + "'", null);
	}
	...
	public ParcoursPassager getParcoursPassager(String p_IdParcourPassager) {
		...
		Cursor c = m_Db.query(DatabaseHelper.TABLE_PARCOURS_PASSAGER, null,
				DatabaseHelper.COL_ID_PARCOURS_PASSAGER + "='" + p_IdParcourPassager + "'", null, null,
				null, null);
		c.moveToFirst();
		if (!c.isAfterLast()) {
	...
	private ContentValues parcourPassagerToContentValues(ParcoursPassager p_ParcourPassager) {
		ContentValues row = new ContentValues();
		row.put(DatabaseHelper.COL_ID_PARCOURS_PASSAGER, p_ParcourPassager.getIdParcoursPassager());
		if (p_ParcourPassager.getListIdPassager() != null) {
			row.put(DatabaseHelper.COL_ID_PASSAGERS, ((ArrayList<String>)p_ParcourPassager.getListIdPassager()).toString());
		}
		row.put(DatabaseHelper.COL_NBR_PASSAGERS, p_ParcourPassager.getNbrPassagers());
		return row;
	}
	
	private ParcoursPassager cursorToParcourPassager(Cursor c) {
		ParcoursPassager pP = new ParcoursPassager();
				pP.SetIdParcoursPassager(c.getString(c.getColumnIndex(DatabaseHelper.COL_ID_PARCOURS_PASSAGER)));
				pP.setListIdPassager(Util.ParseStringToList(c.getString(c.getColumnIndex(DatabaseHelper.COL_ID_PASSAGERS)), ", "));
				pP.setNbrPassagers(c.getInt(c.getColumnIndex(DatabaseHelper.COL_NBR_PASSAGERS)));
;		return pP;
	}
}
